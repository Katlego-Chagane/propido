...
export function MaintenanceConversation({ maintenanceId }: { maintenanceId: string }) {
        ...
        setMessages([
          {
            id: "1",
            content:
              "Hi, I've noticed a leak in the bathroom faucet. It's been dripping constantly for the past few days.",
            sender: {
              name: "Sarah Wilson",
              role: "tenant",
  ...
  if (isLoading) {
    return <div className="flex justify-center items-center h-[600px]">Loading...</div>
  }
  ...
  return (
    <div className="flex flex-col h-[600px] bg-background border rounded-lg overflow-hidden">
      <ScrollArea className="flex-grow">
        <div className="space-y-4 p-4" id="message-container">
          {messages.map((message) => (
            <div
              key={message.id}
              className={`flex ${message.sender.role === "landlord" ? "justify-end" : "justify-start"}`}
            >
              <Card
                className={`max-w-[80%] ${message.sender.role === "landlord" ? "bg-primary text-primary-foreground" : "bg-secondary"}`}
              >
      ...
      <div className="p-4 border-t">
        ...
        {newAttachment && <div className="mt-2 text-sm text-muted-foreground">Attachment: {newAttachment.name}</div>}
      </div>
    </div>
  )
}
